...
clusterLEDTab <- tabPanel(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      radioButtons("clusterButton_outputs", label = strong("Write output to one file per cluster?"), choices = c("Yes","No"), selected = "No", inline = TRUE),
      shinyBS::bsTooltip("clusterButton_outputs", "One file per cluster (Yes) or one file for all clusters (No)"),
      ...
      shinyBS::bsTooltip("clusterInput_directory", "Only required if you want one cluster per file. Must use backward slashes (`/`)!"),

...
motifSearchTab <- tabPanel(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      radioButtons(
        "motifSearchButton_highlight",
        label = strong("Place patterns in parentheses in output?"),
        choices = c("Yes","No"),
        selected = "No",
        inline = TRUE
      ),
      ...
      radioButtons(
        "motifSearchButton_partial",
        label = strong("If multiple patterns, return partial matches?"),
        choices = c("Yes","No"),
        selected = "No",
        inline = TRUE
      ),
      shinyBS::bsTooltip("motifSearchButton_partial", "If No, returned sequences must have ALL patterns."),
      
      # radio buttons for nt or aa searching
      radioButtons(
        "motifSearchButton_motifType",
        label = strong("Type of pattern?"),
        choices = c("Nucleotide","AminoAcid", "String"),
        selected = "Nucleotide",
        inline = TRUE
      ),
      shinyBS::bsTooltip("motifSearchButton_motifType", "Nucleotide option considers degenerate codes; other options do not."),
      
      # select file type for download
      radioButtons(
        "motifSearchDownloadType",
        label = strong("FASTA or CSV download?"),
        choices = c("FASTA", "CSV"),
        selected = "FASTA",
        inline = TRUE
      ),
      shinyBS::bsTooltip("motifSearchDownloadType", "FASTA is required for subsequent modules; CSV retains all features from data table"),
      
      # start button
      actionButton("motifSearchStart", label = h5("Start"), style='padding:11px; font-size:80%'),
      
      # download button
      downloadButton("motifSearchDownload", label = h5("Download"), style='padding:2px; font-size:80%')
    ),

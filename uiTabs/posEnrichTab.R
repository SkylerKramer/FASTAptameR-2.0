  ...
  sidebarLayout(
    sidebarPanel(
      ...
      fileInput(
        ...
        multiple = FALSE,
        placeholder = "Sequence Enrichment CSV file",
        accept = c(".csv")
      ),
      ...
      radioButtons("alphabetMods", label = strong("Do you want to adjust the standard alphabet?"), choices = c("Yes","No"), selected = "No", inline = TRUE),
      ...
      # enrichment range
      sliderInput("slider_enrichmentRange", label = strong("Range of enrichment values:"), min = 0, max = 200, value = c(0,10), step = 1),
      shinyBS::bsTooltip("slider_enrichmentRange", "What is the range of enrichment values to consider?"),
      
      # get sequence type
      radioButtons("posEnrich_seqType", label = strong("Type of sequences?"), choices = c("Nucleotide", "AminoAcid"), selected = "Nucleotide", inline = TRUE),

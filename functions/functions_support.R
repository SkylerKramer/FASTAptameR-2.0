...
#' This function takes a FASTA file (WITH THE FA2 ID) and returns the same information as a data.frame
fa_formatInput <- function(fastaInput = NULL, population = NULL){
  ...
  for(i in 1:ncol(idDF)){
    idDF[,i] <- as.numeric(as.character(idDF[,i]))
  }
